...
normalizers = ['emoticon_normalize', 'repeat_normalize',
...
with st.sidebar:
    ...
    if n1:
        num_repeats1 = st.number_input(
            'num_repeats of Emoticon_normalize', min_value=2, step=1)
    n2 = st.checkbox(normalizers[1])
    if n2:
        num_repeats2 = st.number_input(
            'num_repeats of Repeat_normalize', min_value=2, step=1)
...
if st.button("Preprocess"):
    if n1 or n2 or n3 or n4 or n5:
        if input:
            preprocessed = process(
                backend, input, n1, n2, n3, n4, n5, num_repeats1, num_repeats2)
